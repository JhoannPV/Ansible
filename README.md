...
## 🔍 Integración SonarQube Self-Hosted & Azure DevOps

Esta colección incluye playbooks para desplegar SonarQube self-hosted en MicroK8s y analizar tus repos (frontend React/Vite + backend Node.js/Express) desde Azure Pipelines.

### 1. Despliegue de SonarQube

Playbooks:
- `install_sonarqube.yml`: crea namespace, secret de DB, PostgreSQL y SonarQube.
- `expose_sonarqube_ingress.yml`: expone SonarQube vía Ingress (requiere addon ingress de MicroK8s).

Variables (añadidas al final de `vars/env.yml.template`):
```yaml
SONAR_NAMESPACE: "sonarqube"
SONAR_DB_USER: "sonar"
SONAR_DB_PASSWORD: "cambia_esta_contra"  # cambia en producción
SONAR_DB_NAME: "sonarqube"
SONAR_HOSTNAME: "sonarqube.local"       # usado por el Ingress
```

Desplegar:
```bash
ansible-playbook -i inventory.ini install_sonarqube.yml

# (Opcional) Ingress
microk8s enable ingress
ansible-playbook -i inventory.ini expose_sonarqube_ingress.yml
```

Acceso:
- NodePort (si modificas el Service a NodePort) o port-forward: `microk8s kubectl -n sonarqube port-forward svc/sonarqube 9000:9000`
- Ingress: http://sonarqube.local (añade a `/etc/hosts`: `IP_DEL_MASTER sonarqube.local`).

### 2. Crear Token en SonarQube
1. Inicia sesión (admin/admin la primera vez, cambia contraseña).
2. `My Account > Security > Generate Tokens`.
3. Guarda el token (se usa en la service connection).

### 3. Service Connection en Azure DevOps
1. Instala extensión "SonarQube" si no existe (Organization Settings > Extensions > Marketplace).
2. En el proyecto: `Project Settings > Service connections > New service connection > SonarQube`.
3. Completa:
   - Server URL: `http://sonarqube.local` (o `http://IP:9000` si sin Ingress).
   - Token: el token generado.
   - Nombre: `SonarQube-SelfHosted` (coincide con pipelines).
4. Marca "Grant access permission to all pipelines".

### 4. Pipelines (Ejemplo de Stage Analyze)
Ambos pipelines incluyen un stage `Analyze` antes de construir y desplegar. El Quality Gate debe pasar para continuar.

Frontend (React/Vite/Yarn) – propiedades clave:
```text
sonar.sources=src
sonar.tests=src
...
### 5. Flujo de Análisis
1. Prepare (configura proyecto y propiedades).
2. Build & Test (genera `coverage/lcov.info`).
3. Analyze (sube resultados a SonarQube).
4. Publish (Quality Gate en Azure Pipelines).

### 6. Requisitos del Agente
- Acceso HTTP/HTTPS hacia SonarQube.
- Resolución DNS/hosts del hostname (`/etc/hosts` si no hay DNS).
- Certificado confiable si se usa HTTPS; agrega CA al almacén del sistema si es self-signed.

### 7. Decoración de Pull Requests (Opcional)
En SonarQube: `Administration > ALM Integrations > Azure DevOps`. Configura con un PAT y vincula cada proyecto con su repo para ver comentarios y estado del Quality Gate en los PRs.

### 8. Solución de Problemas
| Error | Causa probable | Acción |
|-------|----------------|--------|
| 401 Unauthorized | Token incorrecto o sin permisos | Regenerar token y actualizar la conexión |
| Connection refused | Ingress/Service caído o IP errónea | Verificar pods y Service: `kubectl -n sonarqube get pods,svc` |
| lcov not found | Tests no generan cobertura | Revisar configuración de Vitest/Jest, ruta `coverage/lcov.info` |
| Quality Gate failed | Código no cumple umbrales | Ajustar reglas o mejorar cobertura/bugs/vulnerabilidades |

### 9. Buenas Prácticas
- Mantén PostgreSQL y SonarQube en su namespace (`sonarqube`).
- Respaldos: copia periódicamente PVC `sonarqube-data-pvc`.
- Revisa logs: `microk8s kubectl -n sonarqube logs deploy/sonarqube`.
